...
const AUTO_SYNC_OPTS_KEY = "auto_sync_options";
...
function StepResult({ title, data }) {
  ...
  // ── Derive aggregate totals for card colour ───────────────────────────────
  let totalFailed  = 0;
  let totalSkipped = 0;
 
  const isPing     = data.connected !== undefined;
  const isLedger   = !!(data.customers || data.suppliers || data.accounts);
  const isInvoice  = !!(data.sales || data.purchase);
  const isVoucher  = !!(data.journalEntries || data.paymentEntries);
  const isBank     = !!data.bankAccounts;
  const isGeneric  = !isPing && !isLedger && !isInvoice && !isVoucher && !isBank;
 
  if (isLedger) {
    totalFailed  += safeNum(data.customers?.failed) + safeNum(data.suppliers?.failed) + safeNum(data.accounts?.failed);
    totalSkipped += safeNum(data.skipped);
  }
  if (isInvoice) {
    totalFailed  += safeNum(data.sales?.failed)    + safeNum(data.purchase?.failed);
  }
  if (isVoucher) {
    totalFailed  += safeNum(data.journalEntries?.failed)  + safeNum(data.paymentEntries?.failed);
    totalSkipped += safeNum(data.journalEntries?.skipped) + safeNum(data.paymentEntries?.skipped);
  }
  if (isBank) {
    totalFailed  += safeNum(data.bankAccounts.failed);
  }
  if (isGeneric) {
    totalFailed  += safeNum(data.failed);
    totalSkipped += safeNum(data.skipped);
  }
 
  const hasFailures = totalFailed > 0 || data.status === "fail" || !!data.error;
  const hasWarnings = !hasFailures && (totalSkipped > 0 || data.status === "warn");
  const bgColor     = hasFailures ? C.redL  : hasWarnings ? C.amberL : C.greenL;
  const borderColor = hasFailures ? C.redB  : hasWarnings ? C.amberB : C.greenB;
  const badgeStatus = hasFailures ? "failed" : hasWarnings ? "warning" : "ok";
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <span style={{ fontFamily: C.title, fontSize: 12, fontWeight: 700, color: C.ink }}>{title}</span>
        <StatusBadge status={badgeStatus} />
      </div>
 
      {/* ── Error text ── */}
      {data.error && (
        <p style={{ fontFamily: C.mono, fontSize: 11, color: C.red, margin: 0 }}>{data.error}</p>
      )}
 
      {/* ── ERPNext Ping ── */}
      {isPing && (
        <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
          <StatPill label="Status"  value={data.connected ? "Connected" : "Failed"}
                    color={data.connected ? "green" : "red"} />
          {data.user      && <StatPill label="User"    value={data.user}             color="blue" />}
          {data.latencyMs && <StatPill label="Latency" value={`${data.latencyMs}ms`} color="blue" />}
        </div>
      )}
      ...
      {isLedger && (
        <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
          {data.customers && <>
            <StatPill label="Cust +"  value={fmt(data.customers.created || 0)} color="green" />
            <StatPill label="Cust ~"  value={fmt(data.customers.updated || 0)} color="blue"  />
            {(data.customers.failed  || 0) > 0 && <StatPill label="Cust ✗"  value={fmt(data.customers.failed)}  color="red"   />}
          </>}
          {data.suppliers && <>
            <StatPill label="Supp +"  value={fmt(data.suppliers.created || 0)} color="green" />
            <StatPill label="Supp ~"  value={fmt(data.suppliers.updated || 0)} color="blue"  />
            {(data.suppliers.failed  || 0) > 0 && <StatPill label="Supp ✗"  value={fmt(data.suppliers.failed)}  color="red"   />}
          </>}
          {data.accounts && <>
            <StatPill label="Acct +"  value={fmt(data.accounts.created  || 0)} color="green" />
            <StatPill label="Acct ~"  value={fmt(data.accounts.updated  || 0)} color="blue"  />
            {(data.accounts.failed   || 0) > 0 && <StatPill label="Acct ✗"  value={fmt(data.accounts.failed)}   color="red"   />}
          </>}
          {(data.skipped || 0) > 0 && <StatPill label="Skipped" value={fmt(data.skipped)} color="amber" />}
        </div>
      )}
      ...
      {isInvoice && (
        <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
          {data.sales && <>
            <StatPill label="Sales +"  value={fmt(data.sales.created    || 0)} color="green" />
            <StatPill label="Sales ~"  value={fmt(data.sales.updated    || 0)} color="blue"  />
            {(data.sales.failed    || 0) > 0 && <StatPill label="Sales ✗"  value={fmt(data.sales.failed)}    color="red"   />}
          </>}
          {data.purchase && <>
            <StatPill label="Purch +"  value={fmt(data.purchase.created || 0)} color="green" />
            <StatPill label="Purch ~"  value={fmt(data.purchase.updated || 0)} color="blue"  />
            {(data.purchase.failed || 0) > 0 && <StatPill label="Purch ✗"  value={fmt(data.purchase.failed)} color="red"   />}
          </>}
        </div>
      )}
 
      {/* ── Vouchers (journalEntries + paymentEntries) ── */}
      {isVoucher && (
        <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
          {data.journalEntries && <>
            <StatPill label="JE +"  value={fmt(data.journalEntries.created  || 0)} color="green" />
            <StatPill label="JE ~"  value={fmt(data.journalEntries.updated  || 0)} color="blue"  />
            {(data.journalEntries.skipped || 0) > 0 && <StatPill label="JE ="  value={fmt(data.journalEntries.skipped)} color="amber" />}
            {(data.journalEntries.failed  || 0) > 0 && <StatPill label="JE ✗" value={fmt(data.journalEntries.failed)}  color="red"   />}
          </>}
          {data.paymentEntries && <>
            <StatPill label="PE +"  value={fmt(data.paymentEntries.created  || 0)} color="green" />
            <StatPill label="PE ~"  value={fmt(data.paymentEntries.updated  || 0)} color="blue"  />
            {(data.paymentEntries.failed  || 0) > 0 && <StatPill label="PE ✗" value={fmt(data.paymentEntries.failed)}  color="red"   />}
          </>}
        </div>
      )}
      ...
      {isBank && (
        <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
          <StatPill label="Created" value={fmt(data.bankAccounts.created || 0)} color="green" />
          <StatPill label="Updated" value={fmt(data.bankAccounts.updated || 0)} color="blue"  />
          {(data.bankAccounts.failed || 0) > 0 &&
            <StatPill label="Failed" value={fmt(data.bankAccounts.failed)} color="red" />}
        </div>
      )}
 
      {/* ── Generic flat (COA, stock, godowns, cost centres, opening balances …) ── */}
      {isGeneric && (
        <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
          {data.created !== undefined && <StatPill label="Created" value={fmt(data.created)} color="green" />}
          {data.updated !== undefined && <StatPill label="Updated" value={fmt(data.updated)} color="blue"  />}
          {(data.skipped || 0) > 0    && <StatPill label="Skipped" value={fmt(data.skipped)} color="amber" />}
          {(data.failed  || 0) > 0    && <StatPill label="Failed"  value={fmt(data.failed)}  color="red"   />}
          {/* synced (bank legacy) */}
          {data.synced !== undefined && data.created === undefined &&
            <StatPill label="Synced" value={fmt(data.synced)} color="green" />}
        </div>
      )}
      ...
      {data.errors?.length > 0 && (
        <div style={{
          ...
          <p style={{ margin: "0 0 4px", fontFamily: C.mono, fontSize: 10, color: C.red, fontWeight: 700 }}>
            Errors ({data.errors.length})
          </p>
          {data.errors.slice(0, 5).map((e, i) => (
            <p key={i} style={{ margin: "2px 0", fontFamily: C.mono, fontSize: 10, color: C.red }}>
              • {typeof e === "object" ? `${e.item}: ${e.error}` : e}
            </p>
          ))}
          {data.errors.length > 5 && (
            <p style={{ margin: "2px 0", fontFamily: C.mono, fontSize: 10, color: C.red, opacity: 0.7 }}>
...
function ErpCredentialsPanel({company,onSaved}){
  ...
  const hasCreds=!!(saved.url&&saved.apiKey&&saved.apiSecret); // must be ABOVE useState
  const [url,setUrl]=useState(saved.url||"");
  const [key,setKey]=useState(saved.apiKey||"");
  const [secret,setSecret]=useState(saved.apiSecret||"");
  const [show,setShow]=useState(!hasCreds); // open if no creds, collapsed if already saved
...
export function SyncToErpNext({companies, onCompanyChange}){
...
useEffect(()=>{
  ...
  const initCo=localStorage.getItem("last_tally_company")||companies?.[0]?.name||"";
  const allMappings=JSON.parse(localStorage.getItem("erp_company_map")||"{}");
  const globalCreds=(() => { try { return JSON.parse(localStorage.getItem("erp_global_creds") || "{}"); } catch { return {}; } })();
  return allMappings[initCo] || globalCreds.erpnextCompany || "";
});
  ...
  function loadCreds() {
    const globalCreds = (() => {
      try { return JSON.parse(localStorage.getItem("erp_global_creds") || "{}"); } catch { return {}; }
    })();
    const perCompany = company ? (loadAllCreds()[company] || {}) : {};
    const resolved = (perCompany.url && perCompany.apiKey && perCompany.apiSecret)
      ? perCompany
      : globalCreds;
    setCompanyCreds(resolved);

    const allMappings = JSON.parse(localStorage.getItem("erp_company_map") || "{}");
    const mapped = company ? (allMappings[company] || "") : "";
    setErpCompany(mapped || globalCreds.erpnextCompany || "");
  }

  loadCreds();

  // Re-load whenever Settings page saves credentials to localStorage
  const interval = setInterval(loadCreds, 1000);
  return () => clearInterval(interval);
}, [company]);
  useEffect(()=>()=>clearTimeout(pollRef.current),[]);

  // Auto-ping on load to show connected email
  useEffect(() => {
    const hasCreds = !!(companyCreds.url && companyCreds.apiKey && companyCreds.apiSecret);
    if (!hasCreds) return;
    const creds = credsOverride();
    if (!creds.erpnextUrl) return;
    fetch(`${BASE_URL}/erpnext/ping`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(creds),
    })
      .then(r => r.json())
      .then(data => setErpPing(data))
      .catch(() => {});
  }, [companyCreds]); // eslint-disable-line
  ...
  // ── FIX 2: On mount — restore auto-sync state from backend ───────────────
  // Sets autoRunning=true FIRST (before anything else can clear it),
  // then restores countdown from backend's authoritative nextRunAt.
  useEffect(()=>{
    fetch(`${BASE_URL}/auto-sync/status`)
      ...
      .then(data=>{
        ...
        // ── Restore interval FIRST so totalMs is correct before autoRunning=true ──
        const match=INTERVALS.find(i=>i.label.toLowerCase()===String(cfg.interval).toLowerCase());
        const restoredIntervalMs=match?match.value:INTERVALS[2].value;
        if(match){setAutoInterval(restoredIntervalMs);setAutoIntervalLabel(match.label);}

        if(cfg.enabled){
          // Set remaining time BEFORE autoRunning so ring has correct totalMs immediately
          if(data.nextRunAt){
          ...
          setAutoRunning(true); // set AFTER interval+remaining are ready

          // Re-inject creds into backend scheduler — never saved to disk for security
        ...
        // Restore last sync result — show step breakdown if available
        if(data.lastSync){
          ...
          if(ls.status!=="running"){
            ...
            setLastAutoResult({
              upToDate:isUpToDate,
              error:ls.status==="failed"?ls.error:null,
              result:{...ls,steps:ls.steps||undefined}
            });
            // Also restore run count and history from backend
            if(ls.startedAt){
              setAutoRunCount(c=>c>0?c:1);
  ...
  // ── FIX 3: Countdown + backend poll ──────────────────────────────────────
  // Removed setAutoRemainingMs(0) from the !autoRunning branch — this was
  // causing the countdown to flash to zero every time you navigated back.
  // Backend's nextRunAt is the source of truth for the remaining time.
  useEffect(()=>{
    if(!autoRunning){
      clearInterval(countdownRef.current);
      setAutoNextRun(null);
      // NOTE: intentionally NOT resetting autoRemainingMs to 0 here —
      // the on-mount fetch restores the real value from backend nextRunAt.
      return;
    }
  ...
  },[autoRunning,autoInterval]); // eslint-disable-line react-hooks/exhaustive-deps -- intentionally only on autoRunning change
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:14,fontFamily:C.sans}}>
      ...
      <div style={card}>
        ...
        {/* FIX 4: removed co&& gate — panel always renders, company="" is fine */}
        {!hasCreds ? (
  <div style={{
    marginBottom: 14, padding: "11px 14px", borderRadius: 9,
    background: C.amberL, border: `1.5px solid ${C.amberB}`,
    display: "flex", alignItems: "center", gap: 10,
  }}>
    <span style={{ fontSize: 16 }}>⚙️</span>
    <p style={{ fontFamily: C.mono, fontSize: 10, color: C.amber, margin: 0, flex: 1 }}>
      No ERPNext credentials found for this company.{" "}
      <strong>Go to Settings → ERPNext Credentials</strong> to connect.
    </p>
  </div>
) : (
  <div style={{
    ...
    <div style={{ flex: 1 }}>
      <p style={{ fontFamily: C.mono, fontSize: 10, color: C.green, margin: 0, lineHeight: 1.6 }}>
        ✓ Connected — <strong>{companyCreds.url}</strong>
      </p>
      {erpPing?.connected && (
        <p style={{ fontFamily: C.mono, fontSize: 10, color: C.green, margin: "2px 0 0" }}>
          Logged in as <strong>{erpPing.user}</strong> · {erpPing.latencyMs}ms
        </p>
      )}
    </div>
  </div>
        ...
        {erpCompany && (
  <div style={{marginBottom:14,padding:"8px 13px",borderRadius:8,background:C.greenL,border:`1px solid ${C.greenB}`}}>
    <p style={{fontFamily:C.mono,fontSize:10,color:C.green,margin:0}}>
      ✓ Syncing to ERPNext company: <strong>{erpCompany}</strong>
    </p>
  </div>
)}
{!erpCompany && (
  <div style={{marginBottom:14,padding:"8px 13px",borderRadius:9,background:C.amberL,border:`1px solid ${C.amberB}`}}>
    <p style={{fontFamily:C.mono,fontSize:10,color:C.amber,margin:0}}>
      ⚠ ERPNext company name not set — go to <strong>Settings</strong> to configure.
    </p>
  </div>
)}
      ...
      <div style={card}>
        ...
        <div style={{marginBottom:13}}>
          ...
          {companies?.length>0?(
            <select value={company} onChange={e=>{setCompany(e.target.value);localStorage.setItem("last_tally_company",e.target.value);onCompanyChange&&onCompanyChange(e.target.value);}} style={{...inp(),cursor:"pointer",appearance:"none"}} onFocus={onFocus} onBlur={onBlur}>
                ...
                {lastAutoResult.upToDate?(
                  ...
                  <div style={{background:C.card,border:`1.5px solid ${C.greenB}`,borderRadius:14,padding:18,display:"flex",flexDirection:"column",gap:11,boxShadow:`0 4px 18px ${C.green}14`}}>
                    ...
                    <div style={{display:"flex",flexDirection:"column",gap:7}}>
                      ...
                      <StepResult title="Bank Accounts" data={lastAutoResult.result.steps.bankAccounts}/>
            ...
            <div style={{background:C.card,border:`1.5px solid ${C.greenB}`,borderRadius:14,padding:18,display:"flex",flexDirection:"column",gap:11,boxShadow:`0 4px 18px ${C.green}14`}}>
              ...
              {result.data.result?.steps&&(
                <div style={{display:"flex",flexDirection:"column",gap:7}}>
                  ...
                  <StepResult title="Bank Accounts"      data={result.data.result.steps.bankAccounts}/>
